...
let outTheta = 58 * Math.PI / 180;
let outPhi = 35 * Math.PI / 180;
...
const topCanvas = (sketch) => {
    ...
    function drawOutPoint() {
        ...
        let xPos = RADIUS * Math.sin(outTheta) * Math.cos(outPhi);
        let yPos = RADIUS * Math.sin(outTheta) * Math.sin(outPhi);
        let zPos = RADIUS * Math.cos(outTheta);
